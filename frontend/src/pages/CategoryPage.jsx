...
export const CategoryPage = () => {
  ...
  if (isCalendarCategory) {
    return (
      <div className="max-w-6xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-8 space-y-4">
        <header className="space-y-2 border-b border-gray-200 pb-2 sm:pb-3">
          <p className="text-[10px] sm:text-xs uppercase tracking-[0.4em] sm:tracking-[0.5em] text-primary">Calendar</p>
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-serif font-bold">NLR NEWS CALENDAR</h1>
          <p className="text-gray-600 text-sm sm:text-base lg:text-lg">Download important documents and schedules</p>
        </header>
        <CalendarPDFViewer />
      </div>
    );
  ...
  if (error || !data || !data.category) {
    // If calendar category fails to load, show PDFs instead
    if (isCalendarCategory) {
      return (
